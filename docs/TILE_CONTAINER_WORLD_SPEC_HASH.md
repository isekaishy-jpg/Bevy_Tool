...
world spec. The hash is not a security feature; it is a quick consistency check.
...
The world spec hash is derived from `world.toml` fields:
- tile_size_meters
- chunks_per_tile
- heightfield_samples
...
The header also stores `region_hash` as FNV-1a 64-bit of the region_id string.
